...
export default async function getAllProjects(request, response, next) {
    ...
    const [refreshToken, errorByGettingRefreshToken] = JWT.verifyToken(rawRefreshToken, process.env.CODE_HUB_REFRESH_SECRET);
    ...
    const [projects, errorByGettingProjects] = await safeAsyncCall(async function () {
        const _id = new Types.ObjectId(refreshToken?._id);
        const filters = !_id ?
            { visibility: "public" } :
            {
                $or: [
