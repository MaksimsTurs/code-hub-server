...
export default async function changeAccountData(request: Request, response: Response, next: NextFunction): Promise<void> {
	const _id: string | undefined = response.locals?.tokens?.accessJWTTToken?._id;
	...
	const [updatedAccount, errorByUpdating] = await safeAsyncCall(async function() {
		return (await mongodb.models.Account.findByIdAndUpdate(
			_id, 
			request.body,
			{ 
				new: true,
