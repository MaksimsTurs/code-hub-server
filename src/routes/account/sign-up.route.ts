...
export default async function signUp(request: Request, response: Response, next: NextFunction): Promise<void> {
	...
	const [newAccount, errorByCreatingAccount] = await safeAsyncCall(async function() {
		...
		const hashedPassword: string = bcrypt.hashSync(password, bcrypt.genSaltSync());
